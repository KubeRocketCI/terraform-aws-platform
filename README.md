...
| <a name="module_aws_ebs_csi_driver_irsa"></a> [aws\_ebs\_csi\_driver\_irsa](#module\_aws\_ebs\_csi\_driver\_irsa) | terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks | 5.18.0 |
...
| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_add_userdata"></a> [add\_userdata](#input\_add\_userdata) | User data that is appended to the user data script after of the EKS bootstrap script | `string` | `""` | no |
...
| <a name="input_cluster_iam_role_arn"></a> [cluster\_iam\_role\_arn](#input\_cluster\_iam\_role\_arn) | A cluster IAM role ARN to run EKS cluster | `string` | n/a | yes |
| <a name="input_cluster_identity_providers"></a> [cluster\_identity\_providers](#input\_cluster\_identity\_providers) | Configuration for OIDC identity provider | `any` | `{}` | no |
| <a name="input_cluster_version"></a> [cluster\_version](#input\_cluster\_version) | EKS cluster version | `string` | `"1.26"` | no |
| <a name="input_demand_desired_nodes_count"></a> [demand\_desired\_nodes\_count](#input\_demand\_desired\_nodes\_count) | The number of on-demand Amazon EC2 instances that should be running in the autoscaling group | `number` | `0` | no |
| <a name="input_demand_instance_types"></a> [demand\_instance\_types](#input\_demand\_instance\_types) | AWS instance type to build nodes for on-demand pool | `list(any)` | <pre>[<br>  {<br>    "instance_type": "r5.xlarge"<br>  }<br>]</pre> | no |
| <a name="input_demand_max_nodes_count"></a> [demand\_max\_nodes\_count](#input\_demand\_max\_nodes\_count) | The maximum size of the on-demand autoscaling group | `number` | `0` | no |
| <a name="input_demand_min_nodes_count"></a> [demand\_min\_nodes\_count](#input\_demand\_min\_nodes\_count) | The minimum size of the on-demand autoscaling group | `number` | `0` | no |
| <a name="input_infra_public_security_group_ids"></a> [infra\_public\_security\_group\_ids](#input\_infra\_public\_security\_group\_ids) | Security group IDs should be attached to external ALB | `list(any)` | `[]` | no |
| <a name="input_platform_domain_name"></a> [platform\_domain\_name](#input\_platform\_domain\_name) | The name of existing DNS zone for platform | `string` | `""` | no |
| <a name="input_platform_name"></a> [platform\_name](#input\_platform\_name) | The name of the cluster that is used for tagging resources | `string` | `""` | no |
| <a name="input_private_subnets_id"></a> [private\_subnets\_id](#input\_private\_subnets\_id) | A list of subnet IDs where the nodes/node groups will be provisioned | `list(any)` | `[]` | no |
| <a name="input_public_subnets_id"></a> [public\_subnets\_id](#input\_public\_subnets\_id) | A list of subnets to place the LB and other external resources | `list(any)` | `[]` | no |
| <a name="input_region"></a> [region](#input\_region) | The AWS region to deploy the cluster into (e.g. eu-central-1) | `string` | `""` | no |
| <a name="input_role_arn"></a> [role\_arn](#input\_role\_arn) | The AWS IAM role arn to assume for running terraform (e.g. arn:aws:iam::012345678912:role/EKSDeployerRole) | `string` | `""` | no |
| <a name="input_role_permissions_boundary_arn"></a> [role\_permissions\_boundary\_arn](#input\_role\_permissions\_boundary\_arn) | Permissions boundary ARN to use for IAM role | `string` | `""` | no |
| <a name="input_spot_desired_nodes_count"></a> [spot\_desired\_nodes\_count](#input\_spot\_desired\_nodes\_count) | The number of spot Amazon EC2 instances that should be running in the autoscaling group | `number` | `1` | no |
| <a name="input_spot_instance_types"></a> [spot\_instance\_types](#input\_spot\_instance\_types) | AWS instance type to build nodes for spot pool | `list(any)` | <pre>[<br>  {<br>    "instance_type": "r5.xlarge"<br>  },<br>  {<br>    "instance_type": "r5.2xlarge"<br>  }<br>]</pre> | no |
| <a name="input_spot_max_nodes_count"></a> [spot\_max\_nodes\_count](#input\_spot\_max\_nodes\_count) | The maximum size of the spot autoscaling group | `number` | `1` | no |
| <a name="input_spot_min_nodes_count"></a> [spot\_min\_nodes\_count](#input\_spot\_min\_nodes\_count) | The minimum size of the spot autoscaling group | `number` | `1` | no |
| <a name="input_ssl_policy"></a> [ssl\_policy](#input\_ssl\_policy) | Predefined SSL security policy for ALB https listeners | `string` | `"ELBSecurityPolicy-TLS-1-2-2017-01"` | no |
| <a name="input_tags"></a> [tags](#input\_tags) | A map of tags to apply to all resources | `map(any)` | `{}` | no |
| <a name="input_vpc_id"></a> [vpc\_id](#input\_vpc\_id) | ID of the VPC where the cluster security group will be provisioned | `string` | `""` | no |
| <a name="input_worker_iam_instance_profile_arn"></a> [worker\_iam\_instance\_profile\_arn](#input\_worker\_iam\_instance\_profile\_arn) | ARN assigned by AWS to the instance profile | `string` | `""` | no |
